...
def train_step(sensor_data_and_labels, sensor_data):
    ...
    #The sensor_data_and_labels is randomly samples sepritly from sensor_data. e.g not the same sampels
    
    noise = tf.random.normal([BATCH_SIZE, noise_dim])

    with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
      generated_drive_commands = generator(noise, training=True)
