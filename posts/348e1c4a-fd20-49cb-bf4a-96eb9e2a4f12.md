...
This is what are GAN structure will look like. The Encoder / Generator is almost exactly the same as the encoder above. The only differences are the addition of the z input and the fact that the labels are not used directly to train it. z is a noise vector (random values), which is required to provide stochasticity to the generator. Even give the decoupling of the labels a neural network will still always produce the same output given the same input. Adding a noise vector allow the generator to produce diffrent drive commands given identical sensor data. This is good, becasue we know that offten time there exist may valid drive commands give 