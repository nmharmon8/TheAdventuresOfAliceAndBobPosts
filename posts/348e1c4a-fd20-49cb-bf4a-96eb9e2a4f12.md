...
This is what are GAN structure will look like. The Encoder / Generator is almost exactly the same as the encoder above. The only differences are the addition of the z input and the fact that the labels are not used directly to train it. z is a noise vector, which is required to provied sc 