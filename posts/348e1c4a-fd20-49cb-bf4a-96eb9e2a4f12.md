...
Note that apply the loss is done using only the trainable variables for the generator and discriminator sepretly. The discriminator network is nesserty for computing loss for the genorator, but that loss must only be applyed to the generator. Remb
