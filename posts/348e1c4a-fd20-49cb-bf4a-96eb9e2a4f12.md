...
4. Sample another random sensor data example and drive command label (X2, C2).
5. Give the discriminator (X2, C2) and (X1, C1).
6. The discriminator should attempt to predict that (X2, C2) came from the collected training data and that (X1, C1) as Generated.

Now lets talk about the loss function for a sec. The discriminator has a loss function focused on classifying if samples are from the Real distribution or from the generated distribution. We want the generator to output valid drive commands given sensor data, which means we want its outputs to be capable of fooling the discriminator. 