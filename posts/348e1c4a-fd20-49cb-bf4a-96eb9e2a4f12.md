...
This is an over simplified example. If you have image data coming from a camera then it makes a lot of sense to use convolutional layers to learning features from the image. If you know the possible range for the output you might want to use an activation function that forces the output into the allowed range. We also have skipped over all the data preprocessing that should be done, such as, normalization, resampling the collection rate, ect...
