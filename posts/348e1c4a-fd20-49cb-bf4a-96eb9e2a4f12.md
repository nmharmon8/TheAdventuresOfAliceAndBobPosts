...
This is what are GAN structure will look like. This is not quite a vanilla GAN, it actually a conditional GAN (more on that later). The Encoder/Generator is almost exactly the same as the encoder above. The only differences are the addition of the z input and the fact that the labels are not directly used to train it. Our variable z, is a noise vector (random values), which is required to provide stochasticity to the generator. We are required to provide stochasticity to the generator. We will use z, which is a noise vector (random values).Even give the decoupling of the labels, a neural network will still always produce the same output given the same input. Adding a noise vector allow the generator to produce different drive commands given identical sensor data. This is good, because we know that often time there exist may valid drive commands.
