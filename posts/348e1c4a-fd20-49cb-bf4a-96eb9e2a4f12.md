...
The most basic approch is to create a leaner incoder that take the input and converts into the driving command. 
...
This is an over simplified example. If you have image data coming from a camera then it make a lot of sense to use convolutional layers to learning features from the image. Also if you know the possible range for the output you might want to use an activation function that forces the output into the allowed range. 
