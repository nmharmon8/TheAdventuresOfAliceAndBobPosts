...
Now look at the discriminator loss function. If you look at `fake_loss = cross_entropy(tf.zeros_like(fake_output), fake_output)` calculating the inverse of the generator using 0 rather then 1. So all the discriminator loss is doing is assing the lable 1 for samples from the real dataset and 0 for samples from the genrator. 