...
A GAN has an addition network called the discriminator. The job of the discriminator is to classify if the input came from the Generator or the actual dataset. That might seem a bit confusing, so lets run through an example of the steps in 