...
Yet there is an inherent limitation to using an encoding approach to imitation learning. It does not matter if the encoder is linear or nonlinear. This is the same problem that autoencoders run into. Reversion to the mean. What dose reversion to the mean look like for robotic control? 
