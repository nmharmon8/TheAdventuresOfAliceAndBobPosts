...
Imagine your robot is approaching an obstacles. It can go left or right, both way are open. Which way should the neural network command the robot to go? Now comes reversion to the mean. Lets assume when the humane driver that we are learning to imitate was driving around, they drove around obstacles sometime to the right and some time to the left. What will the network learn? If the network commands the robot to go right but the label s
