...
The generator is using the output of the discriminator to update how it generates samples. Essentially the gradent from the descriminator expleas how it know the sample was not real so the generator can update its weights to better match the distrubution of real samples.