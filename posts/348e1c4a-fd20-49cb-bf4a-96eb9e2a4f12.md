...
def train_step(sensor_data_and_labels, sensor_data):
    ...
    with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
      generated_drive_commands = generator(generator_input, training=True)
      conditioned_drive_commands = tf.concat([sensor_data, generated_drive_commands], axis=1)
    
      real_output = discriminator(sensor_data_and_labels, training=True)
      fake_output = discriminator(conditioned_drive_commands, training=True)
